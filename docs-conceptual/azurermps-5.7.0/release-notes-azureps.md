...
Instalador de Azure PowerShell 5.7.0: [vínculo](https://github.com/Azure/azure-powershell/releases/download/v5.7.0-April2018/azure-powershell.5.7.0.msi)

Módulo de galería para cmdlets de ARM: [vínculo](https://www.powershellgallery.com/packages/AzureRM/5.7.0)

Para instalar `AzureRM` desde la Galería de PowerShell, ejecute el siguiente comando:

```powershell
Install-Module -Name AzureRM -Repository PSGallery -Force
```

Para actualizar desde una versión anterior de `AzureRM`, ejecute el siguiente comando:

```powershell
Update-Module -Name AzureRM
```

## <a name="changes-since-last-release"></a>Cambios desde la última versión

#### <a name="general"></a>General
* Se ha actualizado a la versión más reciente de Azure ClientRuntime

#### <a name="azurestorage"></a>Azure.Storage
* Se ha corregido el problema por el que los cmdlets upload Blob y upload File producen un error en las máquinas que tienen habilitada la directiva FIPS
    - Set-AzureStorageBlobContent
    - Set-AzureStorageFileContent

#### <a name="azurermbilling"></a>AzureRM.Billing
* Nuevo cmdlet Get-AzureRmEnrollmentAccount
  - para recuperar las cuentas de inscripción

#### <a name="azurermcognitiveservices"></a>AzureRM.CognitiveServices
* Integración con la versión 4.0.0 del SDK de administración de Cognitive Services.
* Se ha agregado la operación Get-AzureRmCognitiveServicesAccountUsage.

#### <a name="azurermcompute"></a>AzureRM.Compute
* `Get-AzureRmVmssDiskEncryptionStatus` admite el estado de cifrado en el nivel de disco de datos
* `Get-AzureRmVmssVmDiskEncryptionStatus` admite el estado de cifrado en el nivel de disco de datos
* Actualización para resistencia a las zonas
* 'New-AzureRmVm' y 'New-AzureRmVmss' (conjunto de parámetros simples) admiten las zonas de disponibilidad.

#### <a name="azurermcontainerregistry"></a>AzureRM.ContainerRegistry
* Se ha desacoplado la dependencia de Commands.Resources.Rest y SDK de ARM/Storage.

#### <a name="azurermdatalakestore"></a>AzureRM.DataLakeStore
* Se ha agregado funcionalidad de depuración
* Se ha actualizado la versión del SDK de ADLS Dataplane a 1.1.2
* Export-AzureRmDataLakeStoreItem: dejan de utilizarse los parámetros PerFileThreadCount, ConcurrentFileCount y se incorpora el parámetro Concurrency
* Import-AzureRMDataLakeStoreItem: dejan de utilizarse los parámetros PerFileThreadCount, ConcurrentFileCount y se incorpora el parámetro Concurrency
* Get-AzureRMDataLakeStoreItemContent: se ha corregido el comportamiento de contenido mayor que 4 MB
* Set-AzureRMDataLakeStoreItemExpiry: se ha incorporado un nuevo conjunto de parámetros SetRelativeExpiry para establecer el tiempo de vencimiento relativo
* Remove-AzureRmDataLakeStoreItem : el parámetro Clean deja de utilizarse.

#### <a name="azurermeventhub"></a>AzureRM.EventHub
* Se ha corregido AlternameName en New-AzureRmEventHubGeoDRConfiguration

#### <a name="azurermkeyvault"></a>AzureRM.KeyVault
* Se han actualizado los cmdlets para incluir escenarios de canalización
* Se han agregado mensajes de desuso para un próximo cambio importante

#### <a name="azurermnetwork"></a>AzureRM.Network
* Se ha corregido el mensaje de error de los cmdlets Network

#### <a name="azurermservicebus"></a>AzureRM.ServiceBus
* Se ha agregado "properties" en CorrelationFilter de Rules para admitir customproperties
* Se ha actualizado la ayuda de New-AzureRmServiceBusGeoDRConfiguration y se ha corregido la salida del cmdlet Rules
* Se han corregido las propiedades de reenvío automático de los cmdlets New-AzureRmServiceBusQueue y New-AzureRmServiceBusSubscription

#### <a name="azurermsql"></a>AzureRM.Sql
* Se ha agregado un nuevo cmdlet "Stop-AzureRmSqlElasticPoolActivity" para admitir la cancelación de las operaciones asincrónicas en grupos elásticos
* Se ha actualizado la respuesta para los cmdlets Get-AzureRmSqlDatabaseActivity y Get-AzureRmSqlElasticPoolActivity para reflejar más información en la respuesta

Cambios desde la última versión: https://github.com/Azure/azure-powershell/compare/v5.6.0-March2018...v5.7.0-April2018

## <a name="560---march-2018"></a>5.6.0 - Marzo de 2018

#### <a name="general"></a>General
* Se corrige el problema con el grupo de recursos predeterminado en CloudShell
* Se corrige el problema por el que se ejecutaban scripts de inicio incorrectos durante la importación del módulo

#### <a name="azurermprofile"></a>AzureRM.Profile
* Se habilita la autenticación de MSI en escenarios no admitidos
* Se agrega compatibilidad para identidades de servicio de administradas definidas por el usuario

#### <a name="azurermanalysisservices"></a>AzureRM.AnalysisServices
* Se ha corregido un problema con los scripts de limpieza en la compilación

#### <a name="azurermcdn"></a>AzureRM.Cdn
* Se ha corregido un problema con los scripts de limpieza en la compilación

#### <a name="azurermcompute"></a>AzureRM.Compute
* "New-AzureRmVM" y "New-AzureRmVMSS" admiten discos de datos
* "New-AzureRmVM" y "New-AzureRmVMSS" admiten imágenes personalizadas por nombre o por identificador
* Característica de análisis de registros
    - Se agregó el cmdlet "Export-AzureRmLogAnalyticRequestRateByInterval"
    - Se agregó el cmdlet "Export-AzureRmLogAnalyticThrottledRequests"

#### <a name="azurermcontainerinstance"></a>AzureRM.ContainerInstance
* Se corrige el problema de conjuntos de parámetros para el registro de contenedor y el montaje de volúmenes de archivos de Azure

#### <a name="azurermdatafactoryv2"></a>AzureRM.DataFactoryV2
* Se ha actualizado ADF .NET SDK a la versión preliminar 0.6.0, que contiene los siguientes cambios:
    - Se han agregado las nuevas actividades LinkedService y DatabricksNotebook a AzureDatabricks
    - Se han agregado las propiedades headNodeSize y dataNodeSize a LinkedService de HDInsightOnDemand
    - Se han agregado LinkedService, Dataset y CopySource para SalesforceMarketingCloud
    - Se ha agregado compatibilidad para SecureOutput en todas las actividades
    - Se ha agregado la nueva propiedad BatchCount a la actividad ForEach, que controla cuántas actividades simultáneas se van a ejecutar
    - Se ha agregado una nueva actividad Filter
    - Se ha agregado compatibilidad con los parámetros de LinkedService

#### <a name="azurermdns"></a>AzureRM.Dns
* Compatibilidad para zonas DNS privadas (versión preliminar pública)
    - Se agrega la capacidad de crear zonas DNS que solo son visibles para las redes virtuales asociadas

#### <a name="azurermnetwork"></a>AzureRM.Network
* Se han actualizado los tipos de modelo para compatibilidad con los cmdlets de DNS.

#### <a name="azurermrecoveryservicessiterecovery"></a>AzureRM.RecoveryServices.SiteRecovery
* Cambios para ASR Azure en Azure Site Recovery (los cmdlets admiten actualmente operaciones de Enterprise a Enterprise, de Enterprise a Azure, de Hyper-v a Azure, de VMware a Azure)
...
* Se ha corregido la ayuda para Remove-AzureRmWebAppSlot
...
* Se han agregado los nuevos comandos siguientes para las operaciones de recuperación geográfica ante desastres.
  - Crear un nuevo alias (configuración de recuperación ante desastres):
    - `New-AzureRmEventHubGeoDRConfiguration`
  - Recuperar un alias (configuración de recuperación ante desastres):
    - `Get-AzureRmEventHubGeoDRConfiguration`
  - Deshabilitar la recuperación ante desastres y dejar de replicar los cambios de los espacios de nombres principales a los secundarios
    - `Set-AzureRmEventHubGeoDRConfigurationBreakPair`
  - Invocar una conmutación por error de recuperación ante desastres y volver a configurar el alias para que apunte al espacio de nombres secundario
    - `Set-AzureRmEventHubGeoDRConfigurationFailOver`
  - Eliminar un alias (configuración de la recuperación ante desastres)
...
* Se han agregado los siguientes nuevos comandos para comprobar el nombre del espacio de nombres y la disponibilidad del nombre o alias de la configuración de la recuperación geográfica ante desastres.
  - Comprobar la disponibilidad del nombre del espacio de nombres o del nombre del alias (configuración de la recuperación ante desastres):
